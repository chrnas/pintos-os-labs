...
struct inode 
  {
    ...
    //Lab 6
    struct semaphore write_sem, read_sem, q_sem;
    int read_count;
...
inode_open (disk_sector_t sector) 
{
  ...
  inode->removed = false;
  //Lab 6
  sema_init(&inode->write_sem, 1);
  sema_init(&inode->read_sem, 1);
  sema_init(&inode->q_sem, 1);
...
{
  //sema_down(&inode->q_sem);
  sema_down(&inode->read_sem);
  inode->read_count++;
  if(inode->read_count == 1){
    sema_down(&inode->write_sem);
  }
  //sema_up(&inode->q_sem);
  sema_up(&inode->read_sem);
