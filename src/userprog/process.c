...
/* Starts a new thread running a user prog#include "threads/vaddr.h"
led (and may even exit)
...
process_execute (const char *file_name) 
{
  ...
  struct c_process_arguments *arguments = malloc(sizeof(struct c_process_arguments));

  struct cp_link* cp_l = malloc(sizeof(struct cp_link));
...
/* Make a copy of FILE_NAME.
  ...
  sema_init(&arguments->load_wait,0);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (file_name, PRI_DEFAULT, start_process, arguments);

  // Wait for child creations and check success or failure
  sema_down(&arguments->load_wait);

  if (tid == TID_ERROR)
    palloc_free_page (fn_copy);

  if(cp_l->creation_success){
    //thread_current()->cp_l = cp_l;
    enum intr_level old_level = intr_disable();
    list_push_back(&thread_current()->child_processes, &cp_l->c_elem);
...
{
  struct c_process_arguments *arguments = (struct c_process_arguments*)arg;
  struct cp_link* cp_l = (struct cp_link *)(arguments->cp_l);
  ...
  success = load (arguments->fn_copy, &if_.eip, &if_.esp);

  cp_l->creation_success = success; 

  struct thread * current_t = thread_current(); 
  cp_l->alive_count = 2;
  cp_l->child_thread = current_t;
  cp_l->child_thread->is_orphan = false;
  cp_l->been_executed = false;
  cp_l->child_tid = current_t->tid;
  current_t->cp_l = cp_l;
  cp_l->exec_wait = malloc(sizeof(struct semaphore));
  
  sema_init(cp_l->exec_wait, 0);
  sema_up(&arguments->load_wait);

  /* If load failed, quit. */
  palloc_free_page (arguments->fn_copy);
  if (!success) {
    thread_exit();
  }
...
{
  for(struct list_elem * it = list_begin(&thread_current()->child_processes); it != list_end(&thread_current()->child_processes); it = list_next(it)){
    struct cp_link *cp_l = list_entry(it, struct cp_link, c_elem);
    if(cp_l != NULL && cp_l->child_tid == child_tid && !cp_l->been_executed){
      sema_down(cp_l->exec_wait);
      cp_l->been_executed = true;
      int exit_stat = cp_l->exit_status;
      return exit_stat;
    }
  }
  return -1;
  ...
  if (kpage != NULL) 
    {
      ...
      if (success){
        ...
        *esp -= sizeof(NULL);
        ...
        //DIVISABLE BY 4 (ADJUST STACK POINTER)
        while(((int64_t)*esp)%4 != 0){
          *esp -= 1;
        }
